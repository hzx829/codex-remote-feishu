...
4. Feishu 群聊 on-demand 路径当前只在这些条件同时成立时从 `R0 Detached` 进入恢复：
...
5. Feishu 群聊 on-demand 启动 headless 时，daemon 会把当前文本 action 作为短生命周期内存 continuation 保存到 `surfaceResumeRuntime`。headless attach 成功后，daemon 先删除 continuation，再绕过 on-demand gate 重新进入普通 ingress apply，使原消息按正常 queue/dispatch 发送；启动失败或 pending timeout 会清掉 continuation，并只把本次恢复失败反馈给当前交互。
