...
6. 后台 auto-restore 触发的 pending headless 也复用同一个 `G1` gate：
   ...
   5. 失败或超时后只发一条恢复失败 notice，并回到 `R0 Detached`；managed headless 连回路径产生的 `headless_restore_*` notice 也必须先进入 daemon recovery episode 过滤，再投递给用户。
