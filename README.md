...
- 推荐的群聊方式：创建“话题”模式群，一个群对应一个工作区，一个飞书话题对应一个 Codex 会话；首次在话题内用 `/list` 绑定工作区，接入旧会话用 `/use`
...
- `/list`：`normal` 模式下选择工作区；在飞书话题内只绑定工作区并准备新 session，在普通私聊/非话题群里仍可继续选择会话；`vscode` 模式下列出在线 VS Code 实例
- 选择方式：`/menu` 和参数卡现在是按钮优先的紧凑卡片，主操作尽量一行一个按钮；`/help` 仍保持文本帮助
- 如果看到旧卡片，请重新发送命令
- `/use`：列出最近可见会话；在飞书话题里用于把已有 Codex session 接入当前话题，选中后会展开 session 详情
- `/threads`：`/use` 的兼容别名
- `/useall`：列出全部可见会话
