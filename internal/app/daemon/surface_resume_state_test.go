...
func TestManagedHeadlessWorkspaceBusyIsReportedOncePerRecoveryEpisode(t *testing.T) {
	...
	first := app.recordManagedHeadlessResumeOutcomeEventsLocked(events, now)
	...
	second := app.recordManagedHeadlessResumeOutcomeEventsLocked(events, now.Add(time.Second))
	if len(second) != 0 {
...
func TestManagedHeadlessFailureWithoutRecoveryStateStillDelivers(t *testing.T) {
	...
	filtered := app.recordManagedHeadlessResumeOutcomeEventsLocked(events, time.Date(2026, 7, 26, 18, 5, 0, 0, time.UTC))
