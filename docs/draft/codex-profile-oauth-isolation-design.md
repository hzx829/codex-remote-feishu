...
> Type: `draft`
> Updated: `2026-07-31`
> Summary: 统一 Codex/Claude 的用户可见 Profile 语义，设计 Codex OAuth 只读 Profile、API Profile 运行认证隔离、跨 Profile 会话恢复以及 Web/飞书管理交互，并明确共享同一 OS 用户时的安全边界。
...
- item ETag 是只由 Profile ID 与 current Revision 生成的 strong opaque validator，不包含 secret 或其摘要；它是 HTTP 并发控制载体，不进入用户可见文案。
...
```go
ResolveCodexProfileRuntime(ref CodexProfileRef) (CodexProfileRuntimeProjection, error)
```

`CodexProfileRef` 是 `{ID, Revision}` 的精确引用，但不是所有选择状态都应保存它：

- bot default 和 route desired selection 只保存 Profile ID，表示“下次 admission 使用 current Revision”。Profile 更新后不需要逐个改写这些选择。
- queue item、pending/active instance、route actual 和 recovery snapshot 保存精确 `CodexProfileRef`，表示“这个已经接纳的动作必须复现当时定义”。resolver 找不到该 Revision 时返回 `profile_revision_unavailable`，不能改取 current。
- prompt 入队前先把 desired Profile ID 解析成 current `CodexProfileRef` 并冻结；Profile 更新与入队必须通过同一 admission/reference owner 串行化，不能在读取 current 与登记引用之间留竞态窗口。
...
- Thread Policy / Effective Thread Contract：前者描述 Profile 默认或 workspace 显式 override，后者记录真实 start/resume 后即将用于 turn 的 model/review/reasoning。
...
3. `/model`、`/reasoning` 快照只把对应字段改成 `explicit`，不会改 Profile 定义。切换回没有快照的 Profile 时恢复该 Profile 自身 policy。
...
6. fixture 只污染临时 Home，不读取用户会话、OAuth 或 API Key；无法证明任一能力时返回 `codex_capability_unsupported`。
...
| `CodexRuntimeResolver` | 把精确 Profile 定义解析成连接合同、线程 policy 和私有启动材料 | `CodexProfileRef`、auth/config evidence、capability fixture | connection contract、thread policy、secret launch material、稳定错误 |
...
端口之间的依赖方向固定为：Catalog/Probe -> Runtime Resolver -> Selection/Instance -> Resume Policy。Web 和飞书只调用 Catalog/Selection 的脱敏 DTO；wrapper/launcher 只接收 connection contract 和私有启动材料；translator 只接收已经解析的 connection/thread policy，不自行读 Catalog 或猜默认值。
...
- workspace 恢复状态分别持久化 desired Profile ID 和上次 actual `CodexProfileRef`/Connection Contract，因此 daemon 重启后既能跟随 current，又能解释上次实际状态。
...
- `route actual`：上次已经接纳/启动的精确 `CodexProfileRef`、Connection Contract 和状态，只用于 compatibility、recovery 与诊断，不能反写 desired。

Profile 切换命令通过 `CodexProfileSelection` 同时更新当前 bot default 和 route desired；当前没有 workspace 时只更新 bot default。surface 字段只是本次交互投影，instance hello 只是 actual state，二者都不是 desired selection 写源。恢复时先把 route desired ID 解析为 current ref，再用 route actual 判断 `preserve_thread_settings` 或 `apply_target_profile`；只有已经冻结的 queue/pending/recovery 动作才能要求重建旧 secret Revision。

Profile 更新不改写 bot/route desired ID。下一条尚未 admission 的 prompt 自动使用 current Revision；已入队、pending 或 running 动作继续使用精确旧 ref。这样旧 Revision 既不会被 desired pin 永久占住，也不会在 Web 保存瞬间改写正在执行的任务。
...
模型、review model、reasoning 由每个 thread 的 policy/effective contract 比较，不进入 child 兼容判定。一个兼容 child 可以承载同一连接上不同 workspace snapshot 的 thread；Profile 模型默认值更新后，下一次 admission 使用新 policy，无需仅为改模型或改名称重启认证进程。

因此：

- 两个实例可以在不同工作区使用不同 Profile。
- 同一个 API Profile 改 Key/端点后旧 child 不再兼容；只改名称或 thread 默认值时，可复用连接但不能复用旧 thread policy。
- 已运行 turn 不因 Web 保存被强行中断；下一次 admission 解析 current Revision，并按 Connection Contract 决定复用或重建。
...
- 删除 API Profile 前，Catalog 必须通过统一 Reference Index 检查 bot default、route desired、active/pending instance、route actual、休眠 persisted surface/workspace、workspace+Profile override 和仍冻结在 queue item 中的精确 ref。任一引用存在都返回 `profile_in_use` 和脱敏引用清单，要求用户先切换或清空；不能只检查 active 进程，也不能删除后把悬空引用自动回退到 native。
...
`CodexResumePolicy` 必须一次性决定 Provider、model、review model 和 reasoning 的 policy。调用方不能先补 `modelProvider`，再由 translator 无规则地猜剩余字段。显式 policy 的 start/resume 使用相同形态：
...
`modelProvider` 和 `model` 使用 typed 参数，reasoning/review model 放入 `config`；不得把四项都塞入非类型化 map。每次 `thread/start` / `thread/resume` 后核对响应中的实际 Provider、model 以及 reasoning 与目标合同。`apply_target_profile` auto policy 可因目标 Codex config 返回非空 reasoning，也可返回 `null` 并继续交给 Codex 的模型 metadata/Provider 默认解析，两者都必须保留 `ReasoningMode=codex_default`，再由 `ReasoningSource` 和 response actual 区分，不能把前者伪装成 Remote 显式 override。`preserve_thread_settings` 带回的值则保留 `preserved_observed` mode。review model 若协议不回显，则在合同中保留 mode，`config/read` 只作为非原子观察证据，受控 fixture 验证行为；不能假装已由 response 证明具体 actual 或跨重启精确冻结 review model。
...
- API Profile 的 frozen Revision 由 Catalog retained record 支撑，queue 不复制 Key。OAuth/native 的旧外部认证或本机配置若已变化且无法复现，dispatch 返回 `profile_revision_unavailable` 并保留可诊断目标；不得把旧队列静默重绑到当前账号/配置。
...
4. 提交目标 desired Profile ID，并将当前 route 标为 `switching`；本次切换 episode 同时冻结当时的 current `CodexProfileRef`，从这一步开始失败也不自动回退旧 Profile。
...
不显示保存、删除和“覆盖认证”入口。重新登录属于本机 Codex 任务，错误态只提供可执行提示，不在 Web 中收集 OAuth token。自定义 ChatGPT 部署显示“当前仅可通过本机默认使用此配置”，不能把它误写成退出登录，也不能提供一个必然失败的 OAuth 启动按钮。
...
必须覆盖：加载中、探测中、已检测到 OAuth、OAuth 已退出、暂时无法检测、已检测到但属于不支持的自定义部署、从未发现 OAuth、Profile 保存中、保存成功、删除冲突、版本过旧、读取配置失败。
...
所有 list/write response 都使用 redacted summary。list body 的每个可写 item 通过 `etag` 字段提供自己的 current strong ETag，单项 write response 再通过 HTTP `ETag` header 返回同一值；只读项不需要伪造可写 ETag。PUT/DELETE 缺少 precondition 返回 428，不匹配返回 412 并返回 current item ETag；业务重名/占用仍使用 409。对 native/oauth 执行 PUT/DELETE 返回稳定只读错误码。references API 只返回脱敏引用类型、可识别名称和阻塞原因，不返回 secret、内部路径或 runtime payload。
...
- bot default 与 route desired 中的 `CodexProviderID` 迁移为浮动 `CodexProfileID`；queue/pending/route actual 等已经接纳的状态迁移为 `CodexProfileRef{ID, Revision: 1}`。不能把所有旧字段机械替换成同一种 ref。
...
| route actual Profile ref/contract | runtime manager | durable recovery/route state | recovery、compatibility、诊断 | 反向覆盖 desired；静默重绑 current |
...
| queue frozen Profile ref/thread policy | queue owner | queue 生命周期 | dispatch、Profile Reference Index | 入队后随 Profile 编辑变化；复制 secret 到 queue |
...
Profile Reference Index 是 Catalog 删除与 revision GC 的唯一引用视图。每个 queue、dispatch、route actual、pending/active child 和 recovery owner 持有独立 lease，不能因为 child provenance 已 rebind 就释放仍在运行的旧 Thread Policy 引用。更新事务必须先写新 Revision/current pointer，旧 Revision 默认继续保留；rebind 必须先 acquire 新 lease、提交 owner state，再 release 旧 lease；GC 只能在所有 owner 已持久提交释放后执行。daemon 启动时先从 durable queue/pending/runtime/recovery state 重建 lease，再清理孤立旧 Revision，不能按进程内计数直接删除。`CurrentRevision` 永不作为 retained history 被 GC；删除整个 Profile 仍必须先通过完整引用检查。

### 14.5 Expand / migrate / cutover

迁移必须在 daemon 启动写流量开放前由单一 coordinator 执行：

1. `expand`：新增 `codex.profiles[]`、OAuth descriptor、浮动 desired selection、精确 frozen ref、Connection Contract 和 Thread Policy schema；旧运行时仍不读取新字段。
...
3. `migrate`：`codex.providers[]` 一对一变成 `kind=api`、Revision=1、CredentialGeneration=1、ConnectionGeneration=1；字段不完整项保持可见但不可启动。旧 `default` 只映射到 `native`，绝不因为检测到 OAuth 自动改成 `oauth`；旧 Provider ID 在 bot/route desired 中映射为 Profile ID，在已经接纳的 queue/pending/actual 中映射为 Revision 1 精确 ref，override key 映射到同 ID Profile；不存在的 ID 保留为显式迁移诊断。
...
3. 建立 Connection Contract、浮动 desired/精确 frozen ref 和 revision retention/GC。
4. 实现 Thread Policy、真实 start/resume 后的 Effective Thread Contract 和 translator resume config，修复跨 Profile exact-thread 恢复。
...
- desired Profile ID 在 Definition 更新后自动解析 current；已入队/active 的精确 ref 保持旧 Revision，不会永久 pin 住 route desired，也不会被 current 覆盖。
...
2. 受支持的官方 OAuth 被投影为可选择、不可编辑、不可删除的 Profile；已检测到但不受支持的自定义部署保留只读状态和可执行提示，不伪装成可用。
...
5. Profile 编辑后，旧 child 不会冒充新的 Key/端点连接；只改名称或 thread 默认时也不会发生无意义重启，下一次 admission 使用新 Thread Policy。
6. 跨 Profile 恢复已有会话时，端点和认证来自目标 Connection Contract，模型和推理强度来自目标 Thread Policy/显式快照，并在发送 prompt 前形成可核对的 Effective Thread Contract。
